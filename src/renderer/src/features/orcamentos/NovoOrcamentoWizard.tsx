...
export default function NovoOrcamentoWizard() {
  ...
  const [meta, setMeta] = useState<Orcamento>({
    nome: '',
    cliente: '',
    responsavel: '',
    dataInicio: '',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(240px,300px)] max-w-7xl mx-auto p-8 gap-6">
        <div>
          ...
          <div className="space-y-6 min-h-[100dvh] pb-24">
            ...
            {step === 3 && (
              <StepPrecoEMargem items={items} fin={fin} meta={meta} setMeta={setMeta} />
            )}
      ...
      <div className="sticky bottom-0 inset-x-0 z-50 backdrop-blur border-t bg-card/70 border-border">
        <div className=" mx-auto px-4 md:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 text-xs sm:text-sm">
            ...
            <span
              className={`hidden sm:inline-flex items-center gap-2 px-2.5 py-1 rounded-xl border border-border text-fg 
                            }`}
            >
              <span className="opacity-70">Descontos:</span>
              <span className="opacity-90">
                {totals.descontoTotal.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL'
                })}
              </span>
              <span className="opacity-60">
                ({Number(fin.descontoPct).toFixed(2)}% +{' '}
                {fin.descontoValor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})
              </span>
            </span>
