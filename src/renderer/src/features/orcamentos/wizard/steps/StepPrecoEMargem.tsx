...
import { Money } from '../../../../../../modules/@shared/domain/valueObjects/Money'
...
export default function StepPrecoEMargem({
  ...
  useEffect(() => {
    setMeta({ ...meta, precoSugerido: new Money(precoSugerido) }) // mantém
  }, [meta.precoAprovado, precoSugerido, setMeta, meta])

  const card = 'bg-card border-border rounded-2xl border p-4'
  const input =
    'px-3 py-2 rounded-xl border border-border bg-input text-fg text-sm outline-none transition focus:ring-2 focus:ring-ring/40'
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
        ...
        <div>
          ...
          <input
            ...
            value={meta.precoAprovado?.toNumber() ?? ''}
            onChange={(e) => setMeta({ ...meta, precoAprovado: new Money(num(e.target.value)) })}
          />
        </div>
      </div>
    </section>
