...
export function KpiCards({ kpis }: { kpis: KPIs }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
      <KPICard title="Total de Clientes" value={kpis.totalClientes} trend={+8} />
      <KPICard title="Clientes Ativos" value={kpis.clientesAtivos} trend={+5} />
      <KPICard title="Limite Total" value={currency(kpis.limiteTotal)} trend={+12} />
      <KPICard title="Faturamento (mês)" value={currency(kpis.faturamentoMes)} trend={+3} />
      <KPICard title="Ticket Médio" value={currency(kpis.ticketMedio)} trend={-2} />
      <KPICard title="Inadimplência" value={`${kpis.inadimplencia.toFixed(1)}%`} trend={-1} />
    </div>
  );
}
