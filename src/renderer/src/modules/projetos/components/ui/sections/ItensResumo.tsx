...
export function ItensResumo({ projeto }: ItensResumoProps) {
  ...
  const analyzeItems = (): ItemAnalysis => {
    ...
    // Definir thresholds baseados na distribuição
    const highThreshold = totalValue * 0.15 / itemCount // Itens que representam > 15% do valor médio
    const lowThreshold = totalValue * 0.05 / itemCount  // Itens que representam < 5% do valor médio
  ...
  const getCategoriesChartData = () => {
    return categoriesData.slice(0, 6).map((cat, index) => ({
        ...
        <div className="bg-card border border-border rounded-lg p-3 shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.dataKey === 'quantidade' ? `Qtd: ${entry.value}` : formatCurrency(entry.value)}
            </p>
          ))}
        </div>
      )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-card border-border rounded-2xl border p-6">
          <div className="flex items-center gap-2 pb-4 border-b border-border mb-4">
            ...
            <h3 className="text-lg font-semibold text-fg">Distribuição por Densidade de Valor</h3>
          </div>
            ...
            {categoriesData.slice(0, 3).map((category, index) => (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs opacity-70">Participação:</span>
